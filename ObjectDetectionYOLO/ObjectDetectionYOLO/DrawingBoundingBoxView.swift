...
class DrawingBoundingBoxView: UIView {
    ...
    public var predictedObjects: [VNRecognizedObjectObservation] = [] {
        didSet {
            let newlyFound = Set(predictedObjects.map { $0.label })
            findNewItem(newlyDetected: newlyFound)
    ...
    func createLabelAndBox(prediction: VNRecognizedObjectObservation) {
        ...
        if !requiredItem.contains(labelString!) {
            return
        }
    ...
    func findNewItem(newlyDetected: Set<String?>){
